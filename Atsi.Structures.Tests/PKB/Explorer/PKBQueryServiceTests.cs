...
namespace Atsi.Structures.Tests.PKB.Explorer
{
    public class PKBQueryServiceTests
    {
        ...
        public PKBQueryServiceTests()
        {
            ...
            // Konfiguracja mocka
            _mockStorage.SetupGet(s => s.Procedures)
                .Returns(new Dictionary<string, Procedure>());
            _mockStorage.SetupGet(s => s.Follows)
                .Returns(new Dictionary<int, int>());
            _mockStorage.SetupGet(s => s.Parent)
                .Returns(new Dictionary<int, int>());
            _mockStorage.SetupGet(s => s.Modifies)
                .Returns(new Dictionary<int, HashSet<string>>());
            _mockStorage.SetupGet(s => s.Uses)
                .Returns(new Dictionary<int, HashSet<string>>());
            
            _service = new PKBQueryService();
            
            // Ustawienie prywatnego pola _db przez refleksję
            var dbField = typeof(PKBQueryService).GetField("_db", 
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            dbField.SetValue(_service, _mockStorage.Object);
        }

        [Fact]
        public void GetProcedure_ReturnsProcedure_WhenExists()
        {
            // Arrange
            var statements = new List<Statement>();
            var expected = new Procedure("test", statements);
            _mockStorage.Setup(s => s.GetProcedure("test")).Returns(expected);
